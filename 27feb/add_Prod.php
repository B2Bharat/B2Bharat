...
if(isset($add_Prod)) {
	...
	$exp_time=date("Y-m-d", strtotime($exp_time));
	
	$vdeo=trim(addslashes($vdeo));
	
	$color=trim(addslashes($color));
	$prod_group_id=trim(addslashes($prod_group_id));
	...
	if($prod_name=="" ) {
		...
		$set.=",prod_group_name='$prod_group_name'"." ".",prod_group_id='".$_SESSION['user']."'";
               // $set.=",prod_group_name='$prod_group_name'"." ".",prod_group_id='$prod_group_id'";
