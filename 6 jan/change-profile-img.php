...
if(isset($_imgupd)){
	...
	if($_FILES['profile_pic']['tmp_name'] != "" || $_FILES['profile_pic']['tmp_name'] != null)
	{
		...
		if($image_width<200 || $image_height<200) { 
			echo "<br/><center style='color:red;'>Too small! You are upload ($image_width x $image_height) image. it must be (200 x 200) or grater..</center>"; 
			echo "<script>var goback = setTimeout(function(){window.history.back();},5000);</script>";	
			exit;
		}
                ...
                <form action="change-profile-img" method="POST" enctype="multipart/form-data">
					...
					<div class="col-md-6">
						...
						<span class="btn fileinput-button " style="font-size:18px; font-weight:600;">
                            ...
                            <input type="file" name="profile_pic" id="profile_pic" accept="image/*" onchange="img_validate('profile_pic',200,200,1,1,'img_div')">
                        </span>
						...
						<br/>
						<div class="space row">
						<input type="submit" value="Update" name="_imgupd" class="btn btn-primary" onclick="img_validate('profile_pic',200,200,1,1,'img_div');"/>
						</div>
					</div>
				</form>
			</div>
