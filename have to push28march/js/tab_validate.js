...
$("#btn-bl1").click(function(){
	...
	if(!checkLength( $("#BL_off_name").val(),50) ){$("#BL_off_name").focus(); swal('Required!','Name should be 1 to 50 latters','error'); Err++;}
	 else if(isEmpty($("#maincat").val())){swal('Required!','Select main category!','error'); Err++;}
...
$("#btn-bl2").click(function(){
	var Err=0;
        if(isEmpty($("#BL_d_price").val()) ){$("#BL_d_price").focus(); swal('Required!','Desired price cannot be empty!','error'); Err++;}
         else if(isEmpty($("#BL_d_price_unit").val())){swal('Required!','Select Unit!','error'); Err++;}
         else if(isEmpty($("#BL_quantity").val()) ){$("#BL_deli_duration").focus(); swal('Required!','Expected Quantity  cannot be empty!','error'); Err++;}
	  else if(isEmpty($("#BL_quantity_unit").val())){swal('Required!','Select Unit!','error'); Err++;}
          else if(isEmpty($("#BL_buy_capacity").val()) ){$("#BL_buy_capacity").focus(); swal('Required!','Approximate order value cannot be empty!','error'); Err++;}
	//if(!checkLength( $("#BL_off_name").val(),50) ){$("#BL_off_name").focus(); swal('Required!','Name should be 1 to 50 latters','error'); Err++;}
	//else if(isEmpty( $("#BL_ex_date").val()) ){$("#BL_ex_date").focus(); swal('Required!','Date cannot be empty','error'); Err++;}
...
$("#btn-bl3").click(function(){
	var Err=0;
    if(!checkLength( $("#BL_desc").val(),500) ){$("#BL_desc").focus(); swal('Required!','Description should be 1 to 500 latters','error'); Err++;}
...
$("#btn-sl1").click(function(){
	var Err=0;
	if(!checkLength($("#SL_off_name").val(),50) ){$("#SL_off_name").focus(); swal('Required!',' name should be between 1 to 50 letters ','error'); Err++;}
...
$("#btn-sl2").click(function(){
	...
	var Err=0;
        ...
        if(isEmpty($("#SL_base_price").val())){$("#SL_C_code").focus(); swal('Required!',' Base price Required!','error'); Err++;}
        else if(isEmpty($("#SL_d_price_unit").val())){$("#SL_d_price_unit").focus(); swal('Required!','Unit cannot be empty!','error'); Err++;}
           ...
           else if(isEmpty($("#SL_quantity_unit").val())){$("#SL_quantity_unit").focus(); swal('Required!','Unit cannot be empty!','error'); Err++;}
           else if(isEmpty($("#SL_size").val())){$("#SL_size").focus(); swal('Required!','Size cannot be empty!','error'); Err++;}
     else if(isEmpty($("#SL_size_unit").val())){$("#SL_size_unit").focus(); swal('Required!','Unit cannot be empty!','error'); Err++;}
...
$("#btn-sl3").click(function(){
	var Err=0;
	 ...
	 if(!checkLength( $("#SL_det_desc").val(),500) ){$("#SL_det_desc").focus(); swal('Required!','Description should be 1 to 500 latters','error'); Err++;}
...
//	var Err=0;
//	
//	if(isEmpty( $("#SL_other").val()) ){$(SL_other).focus(); swal('Required!','Select if(Err===0){return true;}else{return false;} group','error'); Err++;}
//	else if(!ischeckedany("ready_to_publish")){swal('Required!','Ready to Publish?','error'); Err++;}
...
$("#btn-p3").click(function(){
	...
	//lelse if($('#img_div5').attr('src')==''){swal('Required!','Group image is missing','error'); Err++;}
...
$("#btn-p4").click(function(){
	...
	if(!checkLength($("#prod_name").val(),40) ){$("#prod_name").focus(); swal('Required!','Product name should be between 1 to 40 letters ','error'); Err++;}
       ...
       else if(!checkLength($("#b_desc").val(),900) ){$("#b_desc").focus(); swal('Required!','Brief Description should be between 1 to 900 letters ','error'); Err++;}
...
$("#btn-p5").click(function(){
	...
	if(isEmpty($("#max_capacity").val())){$("#max_capacity").focus(); swal('Required!','Enter Maximum Production Capacity!','error'); Err++;}
...
$("#btn-mc1").click(function(){
	...
	if(!checkLength($("#com_name").val(),50)){$("#com_name").focus(); swal('Required!','Company name should be with in 1-60 letters!','error'); Err++;}
         ...
         else if(isEmpty($("#COM_off_size").val())){$("#COM_off_size").focus(); swal('Required!','Select Office Size!','error'); Err++;}
        else if(isEmpty($("#COM_emps").val())){$("#COM_emps").focus(); swal('Required!','Select Employee Size!','error'); Err++;}
     else if(isEmpty($("#COM_annrev").val())){$("#COM_annrev").focus(); swal('Required!','Select Total Annual Revenue!','error'); Err++;}
...
$("#btn-mc3").click(function(){
	...
	if(!checkLength($("#main_product").val(),60)){$("#main_product").focus(); swal('Required!','Main product 1 should be with in 1-60 letters!','error'); Err++;}
     ...
     else if(isEmpty($("#compliance").val())){$("#compliance").focus(); swal('Required!','Select Complaince!','error'); Err++;}
