...
<!--								   <input type="checkbox" name="send_enqs[]" value="<?echo $cemail[0];?>">-->
				  ...
				  <input type="hidden" value="<? echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];?>" name="url"> 
                    <button type="submit" class="btn view-more-btn-3-1" onclick="setToemail('<?echo $cemail[0];?>','<?echo ucfirst($cemail[1]);?>');" name="enq_submit" onclick="eqsing()"> <i class="fa fa-paper-plane" aria-hidden="true"></i> &nbsp; Send Enquiry</button>
                  </div>
                </form>
        </div>
    </div>
