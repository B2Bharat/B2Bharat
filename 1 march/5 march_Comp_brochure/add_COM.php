...
if(isset($COM_submit)) {	
	...
	$COM_pterms=trim(addslashes($COM_pterms));
	...
	if($cid!=''){
		...
		if($isUploaded){
			...
			if($acn!='update'){
                ...
                $NgImg=$user_id;
		$isUploaded = $com_obj->upload_image('COM_brochure',$NgImg,1000,600,5,3,"uploads/company/brochure/",$acn);
		
		if($isUploaded){
			$iname = $com_obj->img_Name;	
			$set.=",COM_brochure='$iname'";
		}else{
			if($acn!='update'){
				echo $com_obj->img_Err; exit;	
			}
