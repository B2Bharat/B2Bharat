      ...
      <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
          <div class="modal-header model-head">
            ...
            <h4 class="modal-title text-center" id="mySmallModalLabel">Send Enquiry</h4>
          </div>
...
<!--    <div class="modal fade bs-example-modal-sm" id="contact-us" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
          <div class="modal-header model-head">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
            <h4 class="modal-title text-center" id="mySmallModalLabel">Contact Supplier</h4>
          </div>
          <div class="modal-body">
            <div class="row">
              <div class="col-md-12">
	    ...
	    <div class="form-group">
                ...
                <div>
                  ...
                  <div class="form-group text-center">
				  <input type="hidden" value="<? echo $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];?>" name="url"> 
                    <button type="submit" class="btn view-more-btn-3-1" name="enq_submit" onclick="eqsing()"> <i class="fa fa-paper-plane" aria-hidden="true"></i> &nbsp;Contact Supplier</button>
                  </div>
                </form>
              </div>
            </div>
	...
	<div class="modal fade bs-example-modal-sm" id="send-multiple-inquiry" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
      <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
          <div class="modal-header model-head">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">×</span></button>
            <h4 class="modal-title text-center" id="mySmallModalLabel">Send Multiple Enquiries</h4>
          </div>
          <div class="modal-body">
            <div class="row">
              <div class="col-md-12">
...
<div class="left_sidebar">
 ...
 <a class="blue" href="<?echo $siteurl;?>/send_requierment">Send Your Buyer Requirement</a> </div>
 
<div class="right_sidebar">
 <a class="mag" href="<?echo $siteurl;?>/complaint_form">May I Help you</a>
 <a class="orange" href="<?echo $siteurl;?>/feedback_form">Send Your Feedback</a>
</div>
...
<script>
	...
	else if(isEmpty($("#inputEmail3").val())){$("#inputEmail3").focus(); swal('Required!','company name missing ','error'); Err++;}
	else if(isEmpty($("#usertype").val())){$("#usertype").focus(); swal('Required!','user type missing','error'); Err++;}
	if(Err===0){
...
$("#btn-reg1").click(function(){
	var Err=0;
		...
		{
			$("#email").focus(); swal('Required!','invalid email! email should be like example@xyz.com!','error'); Err++;
		}
		else if(isEmpty($("#pass").val()))
		{
			$("#pass").focus(); swal('Required!','password missing','error'); Err++;
		}
		else if(!pswd.match(/[A-z]/))
		{
			$("#pass").focus(); swal('Required!','Password should contain atleast one letter, one capital, one number and minimum of eight characters in length','error'); Err++;
		}
		else if(!pswd.match(/[A-Z]/))
		{
			$("#pass").focus(); swal('Required!','Password should contain atleast one letter, one capital, one number and minimum of eight characters in length','error'); Err++;
		...
		{
			$("#pass").focus(); swal('Too short!','password should be at least 8 Letters!','error'); Err++;
		}
...
var postal=$("#postal").val();
	...
	else if(mob.length>10 || mob.length<9){$("#mobile").focus(); swal('Required!','mobile should be 9 or 10 digits !','error'); Err++;}
	else if(isEmpty($("#countryCodedial").val())){$("#countryCodedial").focus(); swal('Required!','please select countrycod !','error'); Err++;}
	else if(isEmpty($(".cnumber").val())){$(".cnumber").focus(); swal('Required!','company number is missing!','error'); Err++;}
	else if(cmob.length>10 || cmob.length<9){$(".cnumber").focus(); swal('Required!','company number should be 9 or 10 digits !','error'); Err++;}
	...
	else if(postal.length>9 || postal.length<3){$("#postal").focus(); swal('Required!','postal code should be greater than 2 and less than 9!','error'); Err++;}
...
function btn_reg3(){
 ...
 var urlregex = new RegExp(
	...
	if(webs!='' && !urlregex.test(webs)){$(".websitecom").focus(); swal('Required!','website is invalid!','error'); Err++;}
	else if(isEmpty($("#cpt").val())){$("#cpt").focus(); swal('Required!','Enter captcha!','error'); Err++;}
...
function bulk_enqs(){	
	var checked = "";
	$("input[name='send_enqs[]']:checked").each(function (){checked += $(this).val()+' ';});
