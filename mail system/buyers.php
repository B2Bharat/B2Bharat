<?
$tpTitle = 'Buyers | B2Bharat.com ,Buyer List,List of Customers,B2bharat Clientel,Wholsaler List';
$pgDesc = 'List of B2Bharat Buyers, List Of traders for Industrial Items,B2B Industrial Wholsaler, Indian Manufacturers List.';
$pgKeywords = 'B2Bharat Industrial Search Result, Product Search Result, Product and Service Information, Customer Requirement, B2B Client List, List Of Enterprises, distributors , traders , Buyer, Seller, Suppliers, Wholesaler, service provider.';
...
$sql="select a.id,a.user_id,a.name,a.country,a.type,a.business_type,a.website,a.phone,a.company_intro,a.create_date,a.logo from company as a, register as b where a.user_id = b.id AND b.mem_pack!='1' AND (a.type='buyer' or a.type='both') AND a.active_status='1' order by id desc";
...
<a target="_blank" href="<?echo $siteurl;?>/company-detail/<?echo strtolower(str_replace(' ','-',$cinfo['name']));?>/<?echo $com_obj->encid($cinfo['id']);?>"><img src="<?echo $logourl;?>" alt="Image" class="img-responsive"></a>
							  ...
							  <?php if($cinfo['website']!='') { ?>
								<a target="_blank" target="_blank" href="<?php $link = $cinfo['website'];$link = (0 === strpos($link, 'http')) ? $link : 'http://' . $link; echo $link;?>"><h3 class="item-price"><?echo ucfirst($cinfo['name']);?></h3></a>
							  <?php  }else{  ?>
...
<!--								<a href="#" class="btn view-more-btn-2-1" data-toggle="modal" data-target="#contact-now" ><i class="fa fa-phone" aria-hidden="true"></i> Contact Now </a>-->
                                ...
                                <span class="hide-number"><? echo $cinfo['phone']; ?></span>
                            </span>
                            <?}?>
								  <span  class="tag  ml20">          
								  ...
								  <button onclick="setToemail('<?echo $cemail[0];?>','<?echo ucfirst($cemail[1]);?>');" class="btn view-more-btn-2-2 ml20" data-toggle="modal" data-target="#send-an-inquiry"><i class="fa fa-paper-plane-o" aria-hidden="true"></i>   Send An Inquiry </button>
								</div>
